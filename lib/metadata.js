...
function buildCapabilities(fields) {
  const fieldNames = Array.isArray(fields)
    ? fields.map(String).sort()
...
function addCapabilities(dataset) {
  return { ...dataset, capabilities: buildCapabilities(dataset.fields) };
}
...
function buildMetadata(datasets, asArray, lastScrapedAt) {
  ...
  if (generated && Array.isArray(generated.datasets)) {
    return {
      ...
      datasets: generated.datasets.map(addCapabilities),
  ...
  const metadata = {
    ...
    datasets: datasetNames(datasets).map(name => addCapabilities(describeDataset(name, asArray))),
  };
...
function buildSummaryMetadata(datasets, asArray, lastScrapedAt) {
  const metadata = {
    ...
    datasetCount: datasetNames(datasets).length,
    datasets: datasetNames(datasets).map(name => addCapabilities(describeDatasetSummary(name, asArray))),
  };
  if (lastScrapedAt) metadata.lastScrapedAt = lastScrapedAt;
  return metadata;
}
...
function getSingleDatasetMetadata(datasets, asArray, lastScrapedAt, name) {
  ...
  if (generated && Array.isArray(generated.datasets)) {
    ...
    if (existing) return addCapabilities(lastScrapedAt ? { ...existing, lastScrapedAt } : existing);
  }
  if (!Object.prototype.hasOwnProperty.call(datasets, name)) return undefined;
  const dataset = addCapabilities(describeDataset(name, asArray));
...
function buildBasicMetadata(datasets, lastScrapedAt) {
  ...
  for (const name of Object.keys(datasets).filter(key => !key.startsWith('_')).sort()) {
    ...
    datasetList.push(addCapabilities({
      name,
      file: `${name}.json`,
