...
describe('GZW Data API', () => {
  it('should return API root on GET /api', async () => {
  ...
  it('should return stats endpoint', async () => {
    ...
    // Each dataset should have a total count
  ...
  it('should paginate results', async () => {
    ...
    assert.ok(body.perPage === 5);
    assert.ok(typeof body.total === 'number');
    assert.ok(typeof body.totalPages === 'number');
  });

  it('should support ?all=true to disable pagination', async () => {
    const { res, getStatus, getBody } = mockRes();
  ...
  it('should search across datasets', async () => {
    const { res, getStatus, getBody } = mockRes();
  ...
  it('should return spec endpoint', async () => {
    const { res, getStatus, getBody } = mockRes();
  ...
  it('should handle smart routes', async () => {
    const { res, getStatus, getBody } = mockRes();
    handler(mockReq('/api/armor?all=true'), res);
    assert.strictEqual(getStatus(), 200);
    const body = getBody();
    assert.ok(Array.isArray(body.data));
    // Armor items should have a category field added by the smart route mutator
  ...
  it('should handle images endpoint', async () => {
    const { res, getStatus, getBody } = mockRes();
  ...
  it('should handle weapon_parts smart route', async () => {
    const { res, getStatus, getBody } = mockRes();
    ...
    // Should have part_category field added by the default mutator
  ...
  it('should return 429 when rate limited', async () => {
    ...
    // Exhaust the rate limit by making many requests to the same IP
    for (let i = 0; i < 100; i++) {
      const { res: r } = mockRes();
    ...
    const { res, getStatus, getBody } = mockRes();
    handler(mockReq('/api'), res);
    // Since rate limit is 100/min, the 101st request might be limited
    // Note: this test is timing-dependent, so we just check the response is valid
